...
contract EduLingo is ReentrancyGuard, Ownable {
    ...
    // Events
    event UserCreated(address indexed user, string[] languages, string[] certifications);
    ...
    function getActiveListings(string memory _language, bool _isTeaching) external view returns (
        ...
        for (uint256 i = 0; i < userListings[msg.sender].length; i++) {
            ...
            if (listing.isActive && 
                ...
                keccak256(bytes(listing.language)) == keccak256(bytes(_language))) {
                count++;
            }
        }
